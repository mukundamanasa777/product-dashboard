...
import { BoardManufacturerFilter } from "./BoardManufacturerFilter";
import { TriggerFilter } from "./TriggerFilter";
import { ScanStatusFilter } from "./ScanStatusFilter";
import type { BoardManufacturerOption, ScanStatus, TriggerSource } from "../types";
 ...
 *
 * Shows every run together (manual + scheduled) with a Trigger column,
 * same as LinkCheckRunsTable — Scan Triggering is one page now, not a
 * split Instant/Scheduled view across two pages/two table instances.
 */
export function ScanRunsTable() {
  ...
  // the same page of results instead of always resetting to page 1. Scan
  // Triggering renders exactly one of these tables on its page, so a plain
  // "page" param never collides.
  const [page, setPage] = useState(() =>
    Number(searchParams.get("page") ?? "1"),
  );

  // Filters aren't URL-synced like page is — clearing them on refresh is
  // an acceptable trade-off for keeping this simple; only page needs to
  // survive a reload/shared link.
  const [triggerSources, setTriggerSources] = useState<TriggerSource[]>([]);
  const [boardManufacturers, setBoardManufacturers] = useState<
  ...
  // Any filter change goes back to page 1 — same reasoning as the products
  // page's filtersKey reset (a stale page number could otherwise land past
  // the end of a now-smaller filtered result set).
  function handleTriggerSourcesChange(next: TriggerSource[]) {
    setTriggerSources(next);
    setPage(1);
  }
  function handleBoardManufacturersChange(next: BoardManufacturerOption[]) {
    setBoardManufacturers(next);
    setPage(1);
  }
  function handleStatusesChange(next: ScanStatus[]) {
    setStatuses(next);
    setPage(1);
  }
  ...
  const { data, isLoading, refetch } = useGetScanRunsQuery({
    triggerSources,
    boardManufacturerIds: boardManufacturers.map((b) => b.id),
    statuses,
    page,
    pageSize: PAGE_SIZE,
  });
        ...
        <Group align="flex-start" gap="xl" wrap="wrap">
          ...
          <Box miw={260}>
            <BoardManufacturerFilter
              selected={boardManufacturers}
              onChange={handleBoardManufacturersChange}
            />
          </Box>
          <Box miw={220}>
            <ScanStatusFilter
